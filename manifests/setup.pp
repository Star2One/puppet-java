define java::setup (
  ...
  if ($ensure == 'present') {
    ...
    exec { "update_jre-${name}":
      cwd     => '/',
      command => "echo 'export JRE_HOME=\$JAVA_HOME/jre' >> ${pathfile}",
      unless  => "grep 'export JRE_HOME=\$JAVA_HOME/jre' ${pathfile}",
    ...
    exec { "update_classpath-${name}":
      cwd     => '/',
      command => "echo 'export CLASSPATH=.:\${JAVA_HOME}/lib:\${JRE_HOME}/lib' >> ${pathfile}",
      unless  => "grep 'export CLASSPATH=.:\${JAVA_HOME}/lib:\${JRE_HOME}/lib' ${pathfile}",
      require => Exec["set_java_home-${name}"],
    }
